...
def main(mu=5, plot_evrything: bool = False) -> Dict[str, Any]:
...
if __name__ == "__main__":
    # mus = range(2, 70+2, 2)
    mus = np.arange(5, 25, 0.2)

    results = []

    for mu in mus:
       result = main(mu=mu, plot_evrything=False)
       results.append(get_mean_total_emf(result))
       print(f"Средняя полная ЭДС для mu={mu}: {results[-1]:.4f} В")
    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
