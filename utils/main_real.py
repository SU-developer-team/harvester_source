# -*- coding: utf-8 -*-
"""
Параллельный сценарий для анализа RMS напряжения и координаты z модели по частотам 2–34 Гц.
...
Br = 1.1     # остаточная магнитная индукция (Тл)
c = R * 0.6    # параметр для расчёта сил, c = 0.8R
...
def total_dB_dz(z, z_coil):
    ...
    return np.sum(dB_dz(z, z_coil))

def ode_system(t, y, accel_values, t_eval):
    z, v, I = y
    if not np.isscalar(z):
    ...
    F_gravity = -m * g
    dv_dt = (F3(z) - F1(z) + F_shaker + F_aero + F_gravity) / m
    dI_dt = (total_dB_dz(z, z_coil) * v - R_resistance * I) / L_inductance
...
def process_frequency(freq: int) -> Tuple[float, float, float, float, float, float, float, float, float, float]:
    ...
    try:
        ...
        sol = solve_ivp(
            lambda t, y: ode_system(t, y, accel_values, t_window),
        ...
        # Вычисление сил
        F1_vals = np.array([F1(zz) for zz in z])
        F3_vals = np.array([F3(zz) for zz in z])
        ...
        for i in range(len(t_window)):
            ...
            F_shaker = -m * a_base
            dv_dt[i] = (F3(z[i]) - F1(z[i]) + F_shaker - (Cf * rho * S * v[i] * abs(v[i])) / 2 - m * g) / m

        total_dB = np.array([total_dB_dz(zz, z_coil) for zz in z])
...
if __name__ == "__main__":
    ...
    if not np.isnan(f_res_total):
        try:
            ...
            sol = solve_ivp(
                lambda t, y: ode_system(t, y, accel_values, t_res),
