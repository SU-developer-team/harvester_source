...
def main(f):
    ...
    e_i__50 = np.array([emf_i(zz, vv, z_coil) for zz, vv in zip(z, v)])

    h_80 = 0.08         # расстояние до верхнего магнита, м
    z_coil_80 = np.linspace(0.0015, h_80 - 0.0015, N_turns)  # координаты, исправлены единицы (1.5 мм -> 0.0015 м)
    e_i__80 = np.array([emf_i(zz, vv, z_coil_80) for zz, vv in zip(z, v)])
    
    # Вычисляем общий масштаб по Y для сравнения
    ymin = min(np.min(e_i__50), np.min(e_i__80))
    ymax = max(np.max(e_i__50), np.max(e_i__80))
    
    # Построение графиков
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 6), sharex=True)
    
    ax1.plot(sol.t, e_i__50, label='Индукционная ЭДС 50мм', color='blue')
    ax1.set_ylabel('ЭДС (В)')
    ax1.set_title(f'Индукционная ЭДС для f = {f} Гц (50мм)')
    ax1.legend()
    ax1.grid(True)
    ax1.set_xlim(0, 1)
    ax1.set_ylim(ymin, ymax)
    
    ax2.plot(sol.t, e_i__80, label='Индукционная ЭДС 80мм', color='orange')
    ax2.set_xlabel('Время (с)')
    ax2.set_ylabel('ЭДС (В)')
    ax2.set_title('Индукционная ЭДС для f = {f} Гц (80мм)')
    ax2.legend()
    ax2.grid(True)
    ax2.set_ylim(ymin, ymax)  # Одинаковый масштаб с верхним графиком
    
    plt.tight_layout()
    plt.savefig('emf_plot.png')  # Сохраняем график в файл
