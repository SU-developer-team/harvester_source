...
class ElectromagneticHarvesterID80mm:
    def __init__(self, folder_path: Optional[str] = None):
        ...
        # self.alpha = 0.00035
        self.alpha = 0.4
        self.device_height = 0.080

        # --- Положение по оси z ---
        self.coil_z_bottom_m = self.magnet_height_m + 0.002 + (self.device_height-self.coil_height_m)/2
        self.coil_z_top_m = self.coil_z_bottom_m + self.coil_height_m 
        self.top_magnet_z_bottom_m = self.coil_z_top_m + 0.002 + (self.device_height-self.coil_height_m)/2
        ...
        # --- Начальные условия ---
        self.z0_m = (self.coil_z_bottom_m + self.coil_z_top_m )/ 2.0
        # Диапазон для 20 гц (-1.100210 м/с .. +1.100210 м/с)
        # Диапазон для 30 гц (-1.180641 м/с .. +1.180641 м/с)
        self.v0_mps = 0
    ...
    def set_base_velocity(self, v_mps: float):
        """### CHANGED: удобный сеттер амплитуды скорости шейкера"""
        self.v0_mps = float(v_mps)
    ...
    def compute_emf(self, z_m, v_mps):
        total_emf = sum(self.coil_turn_area_m2 * self.dB_dz(z_m - (self.coil_z_bottom_m + turn * self.wire_diam_m)) * v_mps for turn in range(self.turns_N)) 
        return total_emf
